...
def clear_write_buffer(write_buffer, vid_writer):
    """Thread que consome a fila de saída e grava no disco."""
    while True:
...
def build_read_buffer(read_buffer, args):
    """Thread que lê frames do vídeo de entrada."""
...
def interpolate_video(in_path, out_path, multi=1, fps_override=None, down=0.25, model=None, device=None):
    """
    ...
    start_new_thread(build_read_buffer, (read_buffer, in_path))
    start_new_thread(clear_write_buffer, (write_buffer, vid_writer))
    ...
    while True:
        ...
        # --- prepara versões small e original ---
        ...
        # tensoriza e normaliza small
        I0_small = torch.from_numpy(last_s.transpose(2,0,1))[None].to(device)
        I1_small = torch.from_numpy(cur_s .transpose(2,0,1))[None].to(device)
        # tensoriza e normaliza original
        I0_orig  = torch.from_numpy(last .transpose(2,0,1))[None].to(device)
        I1_orig  = torch.from_numpy(cur  .transpose(2,0,1))[None].to(device)

        I0_small = I0_small.float()
        I1_small = I1_small.float()
        I0_orig  = I0_orig.float()
        I1_orig  = I1_orig.float()
        ...
        # grava frame original
        write_buffer.put(last)
        frame_count += 1

        # para cada frame intermediário
        for i in range(multi):
            t = (i+1) / (multi+1)

            # estima flow e mask em baixa resolução
            flow_small, mask_small = model.inference(I0_small, I1_small)

            # upsample flow/máscara para full-res e ajusta escala
            scale = H / float(h_s)
            flow_up = F.interpolate(flow_small, size=(H, W),
                                    mode='bilinear', align_corners=True) * scale
            mask_up = F.interpolate(mask_small, size=(H, W),
                                    mode='bilinear', align_corners=True)

            # separa fluxos bidirecionais
            f01 = flow_up[:, :2] * t
            f10 = flow_up[:, 2:] * (1 - t)

            # warp + blend usando o frame ORIGINAL
            w0 = warp(I0_orig, f01)
            w1 = warp(I1_orig, f10)
